...
namespace cppreact {
  ...
  namespace _detail {
    ...
    class key {
      ...
      std::function<void(std::vector<keycode>, bool)> _func; ///< Callback
      ...
      void on_key(keycode k, bool down) {
        ...
        if (_key_pressed == _keys.size()) {
          _func(_original_combo, true);
        }
        if (_key_pressed == 0) {
          _func(_original_combo, false);
        }
      }
      public:
       ...
       */
      key(std::vector<keycode> keys, std::function<void(std::vector<keycode>, bool)> func) {
